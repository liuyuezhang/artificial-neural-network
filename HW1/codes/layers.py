...
class Linear(Layer):
    ...
    def backward(self, grad_output):
        """Your codes here"""
        input = self._saved_tensor
        self.grad_W = np.matmul(input.T, grad_output)   # use matmul here is equivalent to sum along batch
        self.grad_b = np.sum(grad_output, axis=0)  # no need to divide N here, done in loss already
